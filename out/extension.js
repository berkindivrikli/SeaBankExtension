...
const shortcutTreeProvider_1 = require("./shortcutTreeProvider");
const queryExecutor_1 = require("./queryExecutor");
function activate(context) {
    const shortcutManager = new shortcutManager_1.ShortcutManager(context);
    const queryExecutor = new queryExecutor_1.QueryExecutor();
    const treeProvider = new shortcutTreeProvider_1.ShortcutTreeProvider(shortcutManager);
    const treeView = vscode.window.createTreeView('seabankShortcutsList', {
        treeDataProvider: treeProvider,
    });
    context.subscriptions.push(treeView);
    // Add shortcut command
    context.subscriptions.push(vscode.commands.registerCommand('seabank.addShortcut', async () => {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('seabank.editShortcut', async (item) => {
        let keybinding = item?.keybinding;
        if (!keybinding) {
    ...
    context.subscriptions.push(vscode.commands.registerCommand('seabank.deleteShortcut', async (item) => {
        let keybinding = item?.keybinding;
        if (!keybinding) {
            const all = shortcutManager.getAll();
            if (all.length === 0) {
        ...
        const confirm = await vscode.window.showWarningMessage(`Delete shortcut ${keybinding}?`, { modal: true }, 'Delete');
        if (confirm === 'Delete') {
    ...
    for (let i = 0; i <= 9; i++) {
        context.subscriptions.push(vscode.commands.registerCommand(`seabank.runShortcut${i}`, async () => {
